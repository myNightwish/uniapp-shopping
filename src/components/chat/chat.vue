...
async function customerChat() {
	...
	await delay(500);
	const mockResponse = {
		knowledge: "这是来自 mock 的知识点。",
		text: "",
		recommend: "你可能想问: mock 推荐的问题",
	};
	const finalMsg =
		mockResponse.knowledge || mockResponse.text || "你是否想问: " + mockResponse.recommend;
	messages.value.push({
		left: false,
		text: finalMsg,
		time: new Date().getTime(),
	});
	if (mockResponse.recommend) {
		input.value = mockResponse.recommend;
	} else {
		input.value = "";
	}
}

// 使用 mock 数据模拟 chatGPT 的响应
async function chatGPT() {
	...
	await delay(500);
	const mockResponse = {
		chatGPT: "这是来自 mock ChatGPT 的回复。",
	};
	const msg = mockResponse.chatGPT;
	if (msg) {
		messages.value.push({
			left: false,
			text: msg,
